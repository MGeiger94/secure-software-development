...
# converting CSV-formatted data into a list (Smith, N.D.)
...
def getauthsfilesql(uid, authgroups, ftype, fname=None, fkeytag=None):
	...
	agsql = "("  # authgroups
	grpcnt = len(authgroups)  # count groups (Snakify, N.D.)
...
def getauthsfiles(dbconlist, appsql):
	try:
		...
		thiscur = dbhandle.cursor()  # Explanation by Tutorialspoint (N.D.)
...
def testfileownership(dbconlist, ownersql):
	try:
		...
		return None
	return resulttuple  # Explanation by Rosuum (2013)
...
def updatesharedgrp(dbconlist, shgrpsql):
	try:
		...
		dbhandle.commit()  # Explanation by Python Software Foundation (N.D. A)
		dbhandle.close()  
	except Exception as err:
		print(err)
		return None
...
def getgroupdetails(asglist):
	...
	for asg in asglist:
		tmplist = []
		grouprecord = DataGroup.query.filter_by(groupid=asg).first()  # Explanation by SQL Alemey (N.D.)
...
def newresultsdict(resultlist):
	...
	if len(resultlist) > 0:
		for result in resultlist:
			filedate = result[4].strftime("%Y-m%-%d %H:%M:%S")  # Explanation by Python Software Foundation (N.D. B)
			if len(result[2]) > 15:
				keytagdisplay = result [2] [:12] + " ..."
			else:
				keytagdisplay = result[2]
			# Converting it to human-friendly file size
			if result[5] > 1000000:  # Explanation by Amos (N.D.)
...
def newsharedgroups(shrgrpdict):
	...
	for item in shrgrpitems:
		...
		if len(item[1][1]) > 30:
			...
			gdesc = item[1][1]
		gdetails = "{}: {}".format(gname, gdesc)  # Explanation by Kapeli (N.D.)
...
def testuserstrps(ustr):
	strpustr = ustr.strip()  # Explanation by Python Software Foundation (N.D. C)
...
def testfsradio(rb1, rb2):
	if (not isinstance(rb1, str)) and (not isinstance(rb2, str)):  # Explanation by Python Software Foundation (N.D. D)
...
# uuid4 from Python is used due to Privacy concerns (Python Software Foundation, N.D. E)
def getnewuuid():
